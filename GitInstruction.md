...
Это версия нашего файла, в которую мы вносим изменения при работе с ним. Чтобы не менять версию файла при работе с ним, мы создаём новую ветку
с помощью функции git branch branch_name , переходим на неё с помощью функции git checkout branch_name и вносим наши изменения, сохраняем их,делаем комит. После,при одобрении руководства,мы можем перенести их на главную ветку(версию нашего файла на основке которого создавалась новая ветка) путём слияния. Для этого необходимо перенестись на главную ветку с помощью команды git branch branch_name(имя главной ветки) и сделать слияние git merge branch_name(имя ветки), где мы сливаем главну ветку с той,в которой делали изменения. Здесь важно учитывать возможное наличие конфликтов, которые возможно придётся разрешать. После делаем коммит и вуаля-вы хакер.
