...
Для того чтобы начать работу с git, необходимо создать новую папку и открыть её в VSC. 
Создать в этой папке репозиторий командой *git init*.
Далее создать в этой парке файл в котором будет вестись дальнейшая работа.
